...
本文总结当前 `axi/rtl/user_rw_cmd_gen.sv` 和 `native/rtl/user_rw_cmd_gen.sv` 的读写调度机制。两版代码的核心目标相同：在 MIG `ui_clk` 域内，根据写 FIFO 压力、读 FIFO 水位、DDR 已写未读数据量和 replay/backtracking 状态，在写入 DDR 与从 DDR 预取读数据之间仲裁。

当前 RTL 的仲裁口径是 **固定优先级 + 读等待老化保护**。代码已经删除旧的预仲裁层、两级 Grant 结构和方向历史记录，普通读写不再做交替授权。
...
- `wr_level_urgent`：写 FIFO 数据量达到 12288 beat，说明写侧接近危险区，需要禁止普通读并优先写。
...
- `rd_fifo_has_grant_space`：读 FIFO 必须有一个完整读服务窗口的空闲空间，AXI 版至少 256 beat，native 版至少 512 beat。
...
`wr_tail_age_reached` 用来避免不满一组 burst 的尾部数据长期滞留。只要写 FIFO 有数据但一直不够完整 burst，等待达到 `WR_TAIL_AGE_LIMIT = 1024` 个 `ui_clk` 后也允许写出。写 burst 长度仍由 FIFO 数据量、`WR_BURST_NUM` 和 AXI 4KB 边界限制决定。
...
读请求真正打包成 `rd_req.valid` 时，还需要 `rd_fifo_has_grant_space` 为真，并且写侧不能处于 `wr_level_urgent`。
...
```systemverilog
block_for_replay = rp_back_en || (rp_back_en_dly_cnt != 8'd0)
```

作用是：读指针回退期间不发起新的 DDR 事务，避免新读写和 replay 指针更新交叠。状态机在 `RW_IDLE` 或 `RW_ARB` 遇到 `block_for_replay` 时会回到或保持 `RW_IDLE`，直到 replay 阻塞窗口结束。

replay 阻塞只限制新事务发起，不会取消已经被 AXI 或 native MIG 接收的事务。

## 4. 仲裁策略

两版都采用单一仲裁状态：

- `RW_IDLE`：等待校准完成、复位释放和 replay 阻塞结束。
- `RW_ARB`：调用 `choose_grant()` 做一次固定优先级判断。
...
1. 写 urgent -> 写
2. 读 urgent -> 读
3. 读 valid 且 rd_wait_age_reached -> 读
4. 写 valid -> 写
5. 读 valid -> 读
```

因此普通读写同时存在时，默认先服务普通写；只有读侧 urgent，或者读请求连续等待达到 `RD_WAIT_AGE_LIMIT = 2048`，读侧才会抢在普通写之前获得授权。读侧已经没有单独 low 水位。

整体事务优先级可以概括为：

```text
最高优先级：复位 / MIG 未校准完成 / make_data_on_edge
次高优先级：replay/backtracking 阻塞窗口
紧急写：写 FIFO 到达 WR_LEVEL_URGENT
紧急读：读 FIFO 到达 RD_LEVEL_URGENT 或 almost_empty，且写不 urgent
读等待老化：读请求连续等待达到 RD_WAIT_AGE_LIMIT
普通写
普通读
```
...
AXI 版本每次授权对应一个 AXI INCR burst。完整写事务必须等到 B response，因此写服务粒度是：
...
写 burst 长度取写 FIFO 可用数量和 4KB 边界限制的较小值。读 burst 长度取固定服务上限、DDR 可读数量和 4KB 边界限制的较小值；是否可以发起读事务，则由 `rd_fifo_has_grant_space` 保证读 FIFO 至少有一个完整读窗口空间。

AXI 读服务上限：

- 固定 `RD_BURST_NUM = 256` beat。
- 当前 RTL 没有 `RD_GRANT_WR_HIGH = 128` 之类的缩短读窗口逻辑。
...
```text
RW_READ_CMD 中如果 wr_level_urgent 有效，回到 RW_ARB
```
...
- 固定 `RD_BURST_NUM = 512` beat。
- 当前 RTL 没有 `RD_SERVICE_WR_HIGH = 128` 之类的缩短读窗口逻辑。
...
4. 读 FIFO urgent -> 在写不 urgent 时优先补读
5. 读请求等待超时 -> 防止普通读长期饿死
6. 普通写 -> 默认先于普通读
7. 普通读 -> 最低普通优先级
```
