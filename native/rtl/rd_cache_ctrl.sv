...
module rd_cache_ctrl #(
    ...
    typedef enum logic [1:0] {
        RD_IDLE,          // Wait window.
        RD_REQ_EN,        // Read view.
        RD_WAIT_INTERVAL  // Inter-view gap.
    } rd_state_t;
    ...
    assign rd_data_cnt_lim      = (rd_data_cnt == (view_size - 16'd1)); // View tail.
    assign sample_frame_rd_done = user_r_valid & rd_data_cnt_lim;       // View read done.
    assign rd_wr_num_equ        = (rd_view_num == wr_view_num);         // Empty by view.
    assign wr_gt_rd             = (wr_view_num > rd_view_num);          // View available.
    ...
    assign interval_cnt_lim  = (interval_cnt == VIEW_INTERVAL_US); // 90us mark.
    assign ddr_rd_req_t      = (~aurora_asy_fifo_almost_full) &
                               (~idle_process_en) &
                               (~refresh_process_en_dd) &
                               tx_channel_up_dd &
                               wr_gt_rd;                           // Raw read gate.
    assign req_en_state_trig = (req_state == RD_IDLE) & interval_cnt_lim & ddr_rd_req_t; // Start view.
