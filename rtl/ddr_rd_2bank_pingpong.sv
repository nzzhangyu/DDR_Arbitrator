...
module ddr_rd_2bank_pingpong #(
   ...
   parameter int PROG_FULL_THRESHOLD  = 14336,
   ...
   output logic [13:0]           rd_level,
   output logic [13:0]           wr_free_level,
   ...
   localparam logic [14:0] TOTAL_DEPTH_LEVEL =
      15'(BANK_DEPTH * BANK_NUM);
   localparam logic [14:0] PROG_FULL_USED_LEVEL =
      15'(PROG_FULL_THRESHOLD);
   localparam logic [14:0] PROG_FULL_FREE_LEVEL =
      TOTAL_DEPTH_LEVEL - PROG_FULL_USED_LEVEL;

   // CDC ownership.
   // Write side commits filled banks.
   // Read side frees banks only after the committed data has fully drained.
   ...
   // Write bank control.
   // DDR read-return beats are captured into one active bank at a time.
   logic [BANK_NUM-1:0] wr_bank_free;
   logic                wr_bank_sel;
   logic                wr_active;
   logic [ADDR_WIDTH-1:0] wr_addr;
   logic [LEVEL_WIDTH-1:0] wr_fill_level;
   logic [15:0]         wr_idle_timer;
   logic                wr_accept;
   logic                wr_commit_now;
   logic [LEVEL_WIDTH-1:0] wr_fill_level_next;
   logic [14:0]         wr_free_count;

   // Read bank control.
   // Committed banks are drained in bank order with a small output skid buffer.
   ...
   // The skid buffer keeps rd_valid stable while XPM read data is in flight.
   ...
   initial begin
      if (BANK_NUM != 2) begin
         $error("ddr_rd_2bank_pingpong supports exactly two banks.");
      end
      ...
      if (PROG_FULL_THRESHOLD > (BANK_DEPTH * BANK_NUM)) begin
         $error("PROG_FULL_THRESHOLD must not exceed total buffer depth.");
      end
   end
   ...
   // Write side.
   // Commit on full bank, explicit flush, or idle timeout for partial banks.
   assign wr_ready          = wr_active && (wr_fill_level < BANK_DEPTH_LEVEL);
   assign full              = ~wr_ready;
   assign wr_accept         = wr_valid && wr_ready;
   assign wr_fill_level_next = wr_fill_level + LEVEL_WIDTH'(wr_accept);
   assign wr_commit_now     = wr_active &&
                              (wr_fill_level_next != '0) &&
                              ((wr_fill_level_next >= BANK_DEPTH_LEVEL) ||
                               (wr_flush && (wr_fill_level_next != '0)) ||
                               ((wr_fill_level != '0) &&
   ...
   always_comb begin
      ...
      if (wr_active) begin
         wr_free_count = wr_free_count +
                         ({1'b0, BANK_DEPTH_LEVEL} - {1'b0, wr_fill_level});
      end

      for (level_comb_i = 0; level_comb_i < BANK_NUM; level_comb_i++) begin
         if (wr_bank_free[level_comb_i]) begin
            wr_free_count = wr_free_count + {1'b0, BANK_DEPTH_LEVEL};
         end
      end
   end

   assign wr_free_level = (|wr_free_count[14:14]) ? 14'h3fff : wr_free_count[13:0];
   assign prog_full     = wr_free_count <= PROG_FULL_FREE_LEVEL;
      ...
      if (rst) begin
         ...
         if (~wr_active) begin
            ...
            if (wr_accept) begin
               ...
               wr_fill_level <= wr_fill_level_next;
            end

            if (wr_accept || wr_commit_now || (wr_fill_level_next == '0)) begin
               wr_idle_timer <= '0;
            end
            ...
            if (wr_commit_now) begin
               wr_commit_len[wr_bank_sel] <= wr_fill_level_next;
   ...
   // Read side CDC.
   // Commit lengths are stable until the selected bank is released.
   ...
   // Read prefetch.
   // Keep the output stream decoupled from XPM read latency.
   // The read side only advertises valid data after the skid FIFO has a beat to present.
   assign skid_pop       = rd_ready && rd_valid;
   ...
   always_ff @(posedge rd_clk) begin
      if (rst) begin
         ...
         underrun <= rd_ready && (~rd_valid) && empty;
   ...
   assign rd_data    = skid_mem[skid_rd_ptr];
   assign rd_valid   = skid_count != '0;
   assign empty      = ~rd_valid && (rd_visible_count == '0);
   assign prog_empty = rd_visible_count < {1'b0, PROG_EMPTY_LEVEL};
   assign rd_level   = (|rd_visible_count[LEVEL_WIDTH:14]) ?
                       14'h3fff : rd_visible_count[13:0];

   genvar bank_g;
   generate
      for (bank_g = 0; bank_g < BANK_NUM; bank_g++) begin : gen_rd_bank_ram
         xpm_memory_tdpram #(
